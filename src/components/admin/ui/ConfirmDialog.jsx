      ...
      <div className="relative w-full max-w-sm overflow-hidden rounded-2xl border border-white/10 bg-emerald-950/95 shadow-2xl shadow-black/60 backdrop-blur-xl animate-modal-in">
        ...
        <div className="flex items-center justify-end gap-3 border-t border-white/5 bg-white/2 px-6 py-4">
          <button
            onClick={onClose}
            disabled={loading}
            className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/4 px-4 py-2 text-xs font-semibold text-slate-300 transition-all hover:bg-white/10 hover:text-white disabled:opacity-50"
          >
          ...
          <button
            onClick={onConfirm}
            disabled={loading}
            className="flex items-center gap-2 rounded-lg border border-red-500/30 bg-red-500/15 px-4 py-2 text-xs font-bold text-red-300 transition-all hover:bg-red-500/25 hover:text-red-200 disabled:opacity-50"
          >
            {loading ? (
              <>
